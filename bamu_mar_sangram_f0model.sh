;;;
;;; F0 model for bamu_mar
;;;

;;; Load any necessary files here

(set! iiit_us_ash_int_lr_params
      '(
	;; These numbers may be modified the speakers range.
	(target_f0_mean 105)   ;; speaker's mean F0
	(target_f0_std 14)     ;; speaker's range
	;; These number should remain as they are
	(model_f0_mean 170)
	(model_f0_std 34)))

(define (bamu_mar_sangram_targ_func1 utt syl)
 "(bamu_mar_sangram_targ_func1 utt syl)
Simple hat accents."
  (let ((start (item.feat syl 'syllable_start))
        (end (item.feat syl 'syllable_end))
        (ulen (item.feat (utt.relation.last utt 'Segment ) 'segment_end))
        nstart nend fustart fuend fuend fstart fend)
        (set! nstart (/ start ulen))
        (set! nend (/ end ulen))
        (set! fustart '130)
        (set! fuend   '110)
        (set! fstart  (+ (* (- fuend fustart) nstart) fustart))
        (set! fend    (+ (* (- fuend fustart) nend) fustart))
      
    (cond
      ((equal? (item.feat syl "R:Intonation.daughter1.name") "Accented")
       (list
        (list start fstart)
        (list (+ start 0.010) (+ fstart 10 ))
        (list (- end   0.010) (+ fstart 8 ))   
        (list end   fend) 
	))
      ((not (item.next syl))
       (list
	(list end fuend)))
      ((not (item.prev syl))
       (list
	(list start fustart)))
      (t
       nil))))

(define (bamu_mar_sangram::select_f0model)
  "(bamu_mar_sangram::select_f0model)
Set up the F0 model for bamu_mar."

  (Parameter.set 'Int_Method 'General)
  (set! int_general_params (list (list 'targ_func bamu_mar_sangram_targ_func1)))

;  (set! F0start_tree bamu_mar_sangram::start_f0)
;  (set! F0mid_tree bamu_mar_sangram::mid_f0)
;  (set! F0end_tree bamu_mar_sangram::end_f0)
;  (set! int_params
;        '((target_f0_mean 140) (target_f0_std 44)
;          (model_f0_mean 140) (model_f0_std 44)))
;  (Parameter.set 'Int_Target_Method Int_Targets_Tree)
)

(define (bamu_mar_sangram::reset_f0model)
  "(bamu_mar_sangram::reset_f0model)
Reset F0 model information."
  t
)

(provide 'bamu_mar_sangram_f0model)
